...
interface ValueCondition {
  ...
  readonly value2?: number; // Required for 'between' / 'notBetween'
}
```
...
> **Note:** `showValue` is accepted in the condition but is currently a no-op — cell values are always shown alongside data bars and icons.
...
interface IconSetCondition {
  ...
  readonly iconSet: IconSetName; // 'arrows' | 'circles' | 'flags' | 'stars'
  readonly thresholds: readonly { value: number; icon: string }[];
  readonly showValue?: boolean;
}
```

Built-in icon sets:
- **arrows** — `▲` `▶` `▼` (thresholds at 67, 33, 0)
- **circles** — `🟢` `🟡` `🔴` (thresholds at 67, 33, 0)
- **flags** — `🟩` `🟨` `🟥` colored squares (thresholds at 67, 33, 0)
- **stars** — `★★★` `★★☆` `★☆☆` `☆☆☆` (4 levels; thresholds at 80, 60, 40, 0)
...
| `getLayer()` | Access the `ConditionalFormatLayer` render layer instance (returns `null` if plugin not installed) |
