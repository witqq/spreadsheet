...
| `cancel()` | `void` | Cancel ongoing loading, remove overlay |
...
while (loadedRows < totalRows) {
  ...
  while (row < totalRows && performance.now() < deadline) {
    store.bulkGenerate(row, batchSize, columnKeys, generateRow);
    row += batchSize;
  }
...
Each chunk uses `bulkGenerate` with a batch size of 5,000 rows for efficient CellStore writes. The engine re-renders and updates the overlay between chunks.
