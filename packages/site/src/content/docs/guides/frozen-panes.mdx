...
description: Pin rows and columns with FrozenPaneManager for fixed headers, sidebars, and 4-region viewport rendering.
---
...
Spreadsheet supports freezing rows and columns via `FrozenPaneManager`. Frozen rows stay pinned at the top and frozen columns stay pinned on the left as the user scrolls. The renderer splits the viewport into four independent regions: corner (frozen rows + frozen cols), frozen-row strip, frozen-col strip, and main scrollable content.
