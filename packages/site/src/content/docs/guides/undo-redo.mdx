...
Expose undo and redo through toolbar buttons via ref. Use `commandExecute`, `commandUndo`, and `commandRedo` events to track state:
...
function App() {
  ...
  useEffect(() => {
    ...
    const updateState = () => {
      ...
      setCanUndo(cm.canUndo());
      setCanRedo(cm.canRedo());
    };

    engine.on('commandExecute', updateState);
    engine.on('commandUndo', updateState);
    engine.on('commandRedo', updateState);

    return () => {
      engine.off('commandExecute', updateState);
      engine.off('commandUndo', updateState);
      engine.off('commandRedo', updateState);
    };
  }, []);
