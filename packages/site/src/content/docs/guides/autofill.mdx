...
interface DetectedPattern {
  ...
  sourceValues: readonly CellValue[];
  step: number;             // Increment for number sequences, interval in ms for dates
}
```
...
|---|---|---|
| `autofillStart` | `{ sourceRange: CellRange }` | Drag started from fill handle |
| `autofillPreview` | `{ sourceRange: CellRange, fillRange: CellRange \| null, direction: FillDirection \| null }` | Preview of fill area during drag |
| `autofillComplete` | `{ sourceRange: CellRange, fillRange: CellRange, direction: FillDirection }` | Fill operation completed |
