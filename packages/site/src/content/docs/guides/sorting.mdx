...
function App() {
  return (
    <Spreadsheet
      ...
      sortable={true}
      onSortChange={(event) => {
        console.log('Sort changed:', event.sortColumns);
      }}
    />
  );
}
...
|---|---|---|
| `toggleColumn` | `(col: number, multiColumn?: boolean) => SortColumn[]` | Cycle sort on column (none→asc→desc→none) |
...
compareCellValues(valA, valB); // -1 | 0 | 1
```
