...
The component also accepts `className` and `style` props that are applied to the wrapper `<div>` element.
...
import { Spreadsheet, type SpreadsheetRef } from '@witqq/spreadsheet-react';
